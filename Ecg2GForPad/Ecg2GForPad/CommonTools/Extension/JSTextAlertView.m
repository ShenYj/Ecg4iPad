...
- (instancetype)initWithTitle:(NSString *)title handler:(void (^)(JSTextAlertButton * _Nonnull))handler
{
    if (self = [super init]) {
        ...
        _horizontalLine.backgroundColor = [UIColor colorWithHexString:@"e4e4e4"].CGColor;
        [self.layer addSublayer:_horizontalLine];
        
        _verticalLine = [CALayer layer];
        _verticalLine.backgroundColor = [UIColor colorWithHexString:@"e4e4e4"].CGColor;
                ...
                constantWidth: (CGFloat)constantWidth
{
    if (self = [super init]) {
        ...
        _contentSize.width = 300; // default width = 200
        ...
        if (title.length) {
            ...
            _titleLabel.textColor = [UIColor js_colorWithHex:0x999999];
        ...
        if (message.length) {
            ...
            _messageLabel.textColor = [UIColor js_colorWithHex:0x666666];
            ...
            //_messageLabel.size = [_messageLabel sizeThatFits:CGSizeMake(_contentSize.width , MAXFLOAT)];
            _messageLabel.size = CGSizeMake(_contentSize.width - 20, _contentSize.height - 20);
...
- (instancetype)initWithCloseButtonTitle: (nullable NSString *)title
                                 message: (nullable NSString *)message
{
    self = [self initWithTitle:title message:message constantWidth:0];
    
    __weak typeof(self) weakSelf = self;
    JSTextAlertButton *closeButton = [JSTextAlertButton buttonWithTitle:@"关闭" handler:^(JSTextAlertButton * _Nonnull button) {
        [weakSelf hiddenCurrentView];
    }];
...
#pragma mark -
- (void)showComposeViewWithCompeletionHandler:(void (^)())compeletionHandler //NSString *clsName
{
    // 添加到视图
//    UIView *view = [UIApplication sharedApplication].keyWindow.rootViewController.view;
//    [view addSubview:self];
    UIView *view = [JSAlertTool getCurrentVC].view;
    if (!self.superview) {
...
- (void)showCurrentView
{
    ...
    if ( iOS9 ) { // [UIDevice currentDevice].systemVersion.floatValue >= 9.0
